        ...
        if use_lm:
            ...
            for chunk_idx in range(num_chunks):
                ...
                if not result.get("success", False):
                    ...
                    lm_status.append(f"âŒ LM Error: {error_msg}")
                    # Return early with error
                    return GenerationResult(
                        audios=[],
                        status_message=f"âŒ LM generation failed: {error_msg}",
                        extra_outputs={},
                        success=False,
                        error=error_msg,
                    )
                ...
                if lm_chunk_time_costs:
                    ...
                    lm_status.append(f"âœ… LM chunk {chunk_idx+1}: {time_str}")
...
def understand_music(
    ...
    Args:
        ...
        vocal_language: Optional language hint for lyrics generation (e.g., "en", "zh", "ja").
                        Helps guide the LLM to generate lyrics in the specified language.
        
    Returns:
        UnderstandResult with parsed metadata fields and status
        
    Example:
        >>> result = understand_music(llm_handler, audio_codes="<|audio_code_123|>...", vocal_language="en")
        >>> if result.success:
